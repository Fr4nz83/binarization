...
__global__ void test(float* src, float* dst)
{
    // Create the data structure representing a grid of blocks (to be used with composition to sync across blocks).
    grid_group grid = this_grid();
...
int main_new()
{
    ...
    void* args[] = {}; // TODO: passare un vettore di float.
    ...
    for(uint32_t r = 0; r < rounds; r++)
    {
	    ...
	    cudaLaunchCooperativeKernel((void*) test, numBlocksPerSm * deviceProp.multiProcessorCount, numThreads, args);
