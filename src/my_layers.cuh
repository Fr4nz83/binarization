...
bool ConvLayer::initialize_cuDNN()
{
	...
	if (status != CUDNN_STATUS_SUCCESS) return false;
	std::cout << "cuDNN kernel tensor descriptor allocation OK!" << std::endl;
...
void ConvLayer::initialize(const float* img_data, const float* filters, const float* bn)
{
...
ConvLayer* ConvLayer::load_input(const float* img_data, const unsigned batch)
{
