...
__device__ __inline__ void In32LayerBatched(In32LayerParam* p)
{
    ...
    for (int bid=blockIdx.x*32+warpid; bid<gdx*gdy*gw; bid+=gridDim.x*32)
    {
        ...
        float f0 = ((by*32+laneid < (p->input_width)) && (bx*32+wid < (p->input_height))) ?
        		   p->input_gpu[(bx*32+wid)*(p->input_width) + by*32 + laneid] :
				   -1.0f;
...
__device__ __inline__ void In32Conv32Layer(In32Conv32LayerParam* p)
{
    ...
    // Size of the region: 32 * # output_channels.
    volatile float* Csub = (float*) &Cs[warpid * (p->output_channels)];

    // Region of shared memory located beyond the above one used to store the filters values (see below).
    // NOTE: the 32 refers to the maximum number of warps (i.e., 1024 threads) in a thread-block.
    ...
    {
        const int bz = bid / (p->output_width * p->output_height); 						 // Determina l'ID dell'immagine assegnato al warp.
        const int by = (bid % (p->output_width * p->output_height)) / (p->output_width); // Determina la riga dell'output assegnato al warp.
        const int bx = (bid % (p->output_width * p->output_height)) % (p->output_width); // Determina la colonna dell'output assegnata al warp.
            ...
            if((ay >= 0) && (ay < (p->input_height))) // Test su y-coord per considerare solo le regioni dell'input che effettivamente esistono...
            {
                ...
                for (int s=0; s<(p->filter_width); s++)
                {
                    ...
                    if((ax >= 0) && (ax < (p->input_width))) // Test su x-coord per considerare solo le regioni dell'input che effettivamente esistono...
                    {
                        ...
                        // ATTENZIONE: qui si assume che i filtri siano binarizzati (0 = -1, 1 = 1)!
                        // Pertanto, ognuna delle letture da word sottostanti carica 32 bit.
                        // "k" rappresenta il k-esimo blocco di 32 valori.
                        // NOTA: i filtri usati nel
                        for (int k=0; k<ots; k++)
                        {
                            ...
                            // NOTA: stiamo accumulando i valori grazie ai due loop sulle dimensioni del filtro!
        ...
        // Here we save the results of the convolutions associated with the output element currently considered by the warp.
        // NOTE: the results get binarized.
        for (int k=0; k<ots; k++)
        {
            ...
            // NOTE: bz is the index of the image, b
            if (p->save_residual)
            {
...
__device__ __inline__ void In32ConvPool32Layer(In32Conv32LayerParam* p)
{
    ...
    for (int bid = blockIdx.x*32+warpid; bid < 4*(p->output_height)*(p->output_width)*(p->batch); bid += gridDim.x*32)
    {
...
__device__ __inline__ void Conv32Layer(Conv32LayerParam* p)
{
            ...
            (p->output_width) * (p->batch); bid += gridDim.x*32)
    {
        ...
        for (int k=0; k<ots; k++)
        {
            ...
            // Take residual into consideration
